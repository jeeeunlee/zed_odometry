...
b(:, :, g_n) = b(:, :, g_n)*dt;
if g_ft(4, 4, g_n) == 1
    b(:, :, g_n) = b(:, :, g_n) -  dAdj(inv(g_ft(:, :, g_n)), g_Fext(:, :, g_n)) * dt ;
end
c(:, :, g_n) = b(:, :, g_n) + aJ(:, :, g_n) * ad(V(:, :, g_n), g_S(:, :, g_n-1) * i_dq(g_n-1)) * dt;
...
for i = g_n-1 : -1 : 2
    ...
    b(:,:,i)=b(:,:,i)*dt;
    if g_ft(4, 4, i) == 1
        b(:, :, i) = b(:, :, i) - dAdj(inv(g_ft(:, :, i)), g_Fext(:, :, i)) * dt;
    end
    c(:, :, i) = b(:, :, i) + ( aJ(:, :, i) * ad(V(:, :, i), g_S(:, :, i-1) * i_dq(i-1)) )*dt;
    omega(:, i) = g_S(:, :, i-1)' * aJ(:, :, i) * g_S(:, :, i-1);
end

%% Evaluate Accle. of base body in floating body
aJ(:, :, 1) = g_J(:, :, 1) + Transform(invf(:, :, 2), aJ(:, :, 2) - (aJ(:, :, 2) * g_S(:, :, 1) * g_S(:, :, 1)' * aJ(:, :, 2)) / omega(:, 2));
b(:, :, 1) = - dad(V(:, :, 1), g_J(:, :, 1) * V(:, :, 1)) + dAdj(invf(:, :, 2), c(:, :, 2)) + dAdj(invf(:, :, 2), aJ(:, :, 2) * g_S(:, :, 1)) / omega(:, 2) * (i_tau(1) - g_S(:, :, 1)'*c(:, :, 2));
b(:, :, 1) = b(:, :, 1) *dt;
if g_ft(4, 4, 1) == 1
    b(:, :, 1) = b(:, :, 1) - dAdj(inv(g_ft(:, :, 1)), g_Fext(:, :, 1))*dt;
end

dV(:,:,1)=-inv(aJ(:,:,1))*b(:,:,1)+gravity_vector;
